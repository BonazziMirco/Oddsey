...
insert into GIORNATE (WAGERS, WINS)
SELECT * FROM (
    values  (0, 0),
            (0, 0),
            (0, 0)
                    ) AS new_giornate(WAGERS, WINS)
WHERE NOT EXISTS (SELECT 1 FROM GIORNATE LIMIT 1);
...
insert into PREMI (NAME, WINNER)
SELECT * FROM (
    values  ('Biglietto per assistere a una partita della squadra del cuore', null),
            ('Maglia ufficiale di un giocatore della squadra del cuore', null),
            ('Serata in pizzeria insieme alla squadra del cuore', null)
                  ) AS new_premi(NAME, WINNER)
WHERE NOT EXISTS (SELECT 1 FROM PREMI LIMIT 1);

INSERT INTO RECENSIONI (author, text, rating, date)
SELECT * FROM (
    VALUES
        ('moderator#24', 'Great experience, very exciting!', 5, '2024-01-15'),
        ('utente#24', 'Decent service, but could be better.', 3, '2024-01-20')
